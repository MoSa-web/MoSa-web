...
def evaluation_vqvae(out_dir, val_loader, net, writer, it, best_fid, best_div, best_top1,
    ...
    for batch in val_loader:
        ...
        pred_pose_eval, *_ = net(motion, m_length)
...
@torch.no_grad()
def evaluation_vqvae_plus_mpjpe_cbusage(val_loader, net, repeat_id, eval_wrapper, num_joint):
    net.eval()
    ...
    for batch in val_loader:
        ...
        pred_pose_eval, *_, indices = net(motion, m_length)

        for ii in range(len(codebook_usages)):
            unique_indices = torch.unique(indices[ii])
            codebook_usages[ii][unique_indices] += 1
...
def evaluation_transformer(out_dir, val_loader, trans, vq_model, writer, ep, best_fid, best_div,
    ...
    for batch in val_loader:
        ...
        # (b, seqlen)
        pred_quantized = trans.generate(clip_text, m_length, cond_scale, vq_model, temperature=1)
...
def evaluation_transformer_test(val_loader, vq_model, trans, repeat_id, eval_wrapper,
    ...
    for i, batch in enumerate(val_loader):
        ...
        # (b, seqlen, c)
            ...
            for _ in range(30):
                pred_quantized = trans.generate(clip_text, m_length, cond_scale, vq_model, top_k=top_k, top_p=top_p, temperature=temperature)
                pred_motions = vq_model.forward_decoder(pred_quantized)
